...
export default function TableComponent(props) {
  ...
  const fetchAllFloor = async () => {
    const result = await postData("tablebookings/fetch_all_floor", {
  ...
  const fetchAllTable = async (fn, i) => {
    props.setTableNo('')
    props.setFloorNo(fn)
    const result = await postData("tablebookings/fetch_all_table_by_floor", {
      restaurantid: admin.restaurantid,
      floor: fn,
    });
  ...
  const showTable = () => {
    return table.map((item) => {
      return (
        <Paper onClick={()=>handleTableClick(item)}
          elevation={3}
          style={{
            borderRadius: 5,
            width: 80,
